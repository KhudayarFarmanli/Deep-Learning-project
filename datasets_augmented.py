...
class TripletsDataset(BaseDataset):
    ...
    def __init__(self, args, datasets_folder="datasets", dataset_name="pitts30k", split="train", negs_num_per_query=10):
        ...
        if args.data_aug != None:
          if args.night_robust != None:
            if args.color_jitter != None:
    ...
    def __getitem__(self, args, index):
        ...
        if args.constant != None: 
          query   =  transforms.Compose([
                       ...
                       transforms.functional.adjust_brightness(path_to_pil_img(self.queries_paths[query_index], brightness_factor=0.25)),
                       transforms.functional.adjust_contrast(path_to_pil_img(self.queries_paths[query_index], contrast_factor=-0.5)),
                       transforms.functional.adjust_saturation(path_to_pil_img(self.queries_paths[query_index], saturation_factor=2)),
                       transforms.functional.adjust_hue(path_to_pil_img(self.queries_paths[query_index], hue_factor=-0.5)),
                       self.resized_transform,
          ])
        else:
