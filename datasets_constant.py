...
def get_params(brightness, contrast, saturation, hue):
        ...
        if brightness is not None:
            ...
            transf.append(transforms.Lambda(lambda img: transforms.functional.adjust_brightness(img, brightness_factor)))

        if contrast is not None:
            contrast_factor = random.uniform(contrast[0], contrast[1])
            transf.append(transforms.Lambda(lambda img: transforms.functional.adjust_contrast(img, contrast_factor)))

        if saturation is not None:
            saturation_factor = random.uniform(saturation[0], saturation[1])
            transf.append(transforms.Lambda(lambda img: transforms.functional.adjust_saturation(img, saturation_factor)))

        if hue is not None:
            hue_factor = random.uniform(hue[0], hue[1])
            transf.append(transforms.Lambda(lambda img: transforms.functional.adjust_hue(img, hue_factor)))

        random.shuffle(transf)
...
class TripletsDataset(BaseDataset):
    ...
    def __init__(self, args, datasets_folder="datasets", dataset_name="pitts30k", split="train", negs_num_per_query=10):
        ...
        self.bright_improv = args.bright_improv
        ...
        if args.brightness_r is not None:
          self.bright = random.uniform(0, args.brightness_r)
        if args.contrast_r is not None:
          self.contr = random.uniform(0, args.contrast_r)
        if args.saturation_r is not None:
          self.sat = random.uniform(0, args.saturation_r)
        if args.hue_r is not None:
          self.hue = random.uniform(0, args.hue_r)
    ...
    def __getitem__(self, index):
        ...
        query_img = path_to_pil_img(self.queries_paths[query_index])
        query = transforms.functional.adjust_brightness(query_img, brightness_factor = self.bright)
        query = transforms.functional.adjust_contrast(query, contrast_factor = self.contr)
        query = transforms.functional.adjust_saturation(query, saturation_factor = self.sat)
        query = transforms.functional.adjust_hue(query, hue_factor = self.hue)
        query = self.resized_transform(query)

        # query = get_params(0.25, 0.5, 2, -0.5, query_img)
