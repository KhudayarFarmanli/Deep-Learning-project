...
'''transf = transforms.Compose([
    transforms.RandomErasing(p = 0.33),
    transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]),
])'''
...
class TripletsDataset(BaseDataset):
    ...
    def __init__(self, args, datasets_folder="datasets", dataset_name="pitts30k", split="train", negs_num_per_query=10):
        ...
        self.transf = transforms.Compose([
            transforms.RandomErasing(p = args.rand_erasing_prob),
            transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]),
        ])
        self.query_transform = transforms.Compose([
                transforms.RandomPerspective(distortion_scale = args.rand_perspective_dist, p = args.rand_perspective_prob, fill = args.rand_perspective_fill) if (args.rand_perspective_dist and args.rand_perspective_prob) != None else identity_transform,
                self.resized_transform
        ])
        #self.query_transforms_2 = transforms.Compose(self.resized_transform)
    ...
    def __getitem__(self, index):
        ...
        query =  self.transf(query)
        #query = transforms.functional.adjust_brightness(query, brightness_factor = 0.25)
        #query = self.query_transforms_2(query)
