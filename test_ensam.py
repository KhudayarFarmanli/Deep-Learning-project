...
def test(args, eval_ds, model, test_method="hard_resize", pca=None, args1=None, model2=None, test_method_2 = "hard_resize" , pca_2=None):
    ...
    with torch.no_grad():
        ...
        # Obtain database dataset Subset
        database_subset_ds = Subset(eval_ds, list(range(eval_ds.database_num)))
        database_dataloader = DataLoader(dataset=database_subset_ds, num_workers=args.num_workers,
                                        batch_size=args.infer_batch_size, pin_memory=(args.device=="cuda"))
        ...
        if test_method == "nearest_crop" or test_method == 'maj_voting':
            ...
            if args.comparison:
              all_features_2 = np.empty((5 * eval_ds.queries_num + eval_ds.database_num, args1.features_dim), dtype="float32")
        else:
            all_features = np.empty((len(eval_ds), args.features_dim), dtype="float32")
            if args.comparison:
              all_features_2 = np.empty((len(eval_ds), args1.features_dim), dtype="float32")

        # Distinguish between the case with and without comparison
        if args.comparison:
          for inputs, indices in tqdm(database_dataloader, ncols=100):
              features = model(inputs.to(args.device))
              features_2 = model2 (inputs.to(args1.device))
              ...
              if pca_2 != None:
                features_2 = pca_2.transform (features_2)

              all_features[indices.numpy(), :] = features
              all_features_2[indices.numpy(),:] = features_2
        ...
        if args.comparison:
          for inputs, indices in tqdm(queries_dataloader, ncols=100):
              ...
              if test_method == "five_crops":  # Compute mean along the 5 crops
                  features = torch.stack(torch.split(features, 5)).mean(1)
                  features_2 = torch.stack(torch.split(features_2, 5)).mean(1)
              features = features.cpu().numpy()
              features_2 = features_2.cpu().numpy()
              ...
              if test_method == "nearest_crop" or test_method == 'maj_voting':  # store the features of all 5 crops
                  ...
                  all_features_2[indices, :] = features_2
              else:
                  all_features[indices.numpy(), :] = features
                  all_features_2[indices.numpy(), :] = features_2
        else:
    ...
    ## SEPARATE IN VARIABLES THE FEATURES
    queries_features = all_features[eval_ds.database_num:]
    database_features = all_features[:eval_ds.database_num]

    if args.comparison: 
      queries_features_2 = all_features_2[eval_ds.database_num:]
      database_features_2 = all_features_2[:eval_ds.database_num]

    ## COMBINE THE MODELS
    if args.comparison:
      queries_features = np.concatenate([queries_features,queries_features_2], axis=1)
      database_features = np.concatenate([database_features,database_features_2], axis=1)
      print (f'Combined queries features extension is {queries_features.shape}')
      print (f'Combined database features extension is {database_features.shape}')
      print (f'Output dimension: {args.features_dim+args1.features_dim}')
    # APPLY NEAREST NEIGHBORS
    faiss_index = faiss.IndexFlatL2(args.features_dim+args1.features_dim)
