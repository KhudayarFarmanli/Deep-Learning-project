...
def test(args, eval_ds, model, test_method="hard_resize", pca=None):
    ...
    with torch.no_grad():
        ...
        ## MULTI_SCALE DATABASE FEATURES
        # Computing features of database images with multiple resolutions
        if args.multi_scale_test:
          for inputs, indices in tqdm(database_dataloader, ncols=100):
              inputs = inputs.to (args.device)
              image_encoding = model.module.backbone (inputs)
              image_encoding = image_encoding.view(image_encoding.size(0),image_encoding.size(1),-1)

              # Maybe we can add scaleSpace as an argument in args
              scaleSpace = [2,4]
              for l in scaleSpace:
                input_scaled = inputs[:,:,::l,::l]
                
                image_scaled_encoding = model.module.backbone(input_scaled)
                
                m= image_scaled_encoding.view(image_scaled_encoding.size(0),image_scaled_encoding.size(1),-1)
              ...
              vlad_encoding = model.module.aggregation(image_encoding)
              vlad_encoding = vlad_encoding.detach().cpu().numpy()

              if pca != None:
                  vlad_encoding = pca.transform(vlad_encoding)

              all_features [indices.detach().numpy(), :] = vlad_encoding

              del inputs, image_encoding, vlad_encoding
        
        # This computes database's features without multiscale
        else: 
          for inputs, indices in tqdm(database_dataloader, ncols=100):
              features = model(inputs.to(args.device))
        ...
        ## MULTI_SCALE QUERIES FEATURES
        #Compute queries features with multi_scale
        if args.multi_scale_test:
          for inputs, indices in tqdm(queries_dataloader, ncols=100):
            if test_method == "five_crops" or test_method == "nearest_crop" or test_method == 'maj_voting':
                  inputs = torch.cat(tuple(inputs))  # shape = 5*bs x 3 x 480 x 480
            ...
            vlad_encoding = model.module.aggregation(image_encoding)
            if test_method == "five_crops":  # Compute mean along the 5 crops
                  vlad_encoding = torch.stack(torch.split(vlad_encoding, 5)).mean(1)
            vlad_encoding = vlad_encoding.detach().cpu().numpy()
            ...
            if test_method == "nearest_crop" or test_method == 'maj_voting':  # store the features of all 5 crops
                ...
                end_idx   = start_idx + indices.shape[0] * 5
                indices = np.arange(start_idx, end_idx)
                all_features[indices, :] = vlad_encoding
            else:
                all_features [indices.detach().numpy(), :]= vlad_encoding
                
        #Compute queries' features without multi_scale    
        else:
          for inputs, indices in tqdm(queries_dataloader, ncols=100):
              if test_method == "five_crops" or test_method == "nearest_crop" or test_method == 'maj_voting':
                  inputs = torch.cat(tuple(inputs))  # shape = 5*bs x 3 x 480 x 480
